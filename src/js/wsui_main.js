...
const fs = require('fs');
const {clipboard, remote, ipcRenderer, tech} = require('electron');
...
const WalletTechManager = require('./ws_manager');
const config = require('./ws_config');

const wsmanager = new WalletTechManager();
...
function showInitialPage(){
    ...
    let versionInfo = document.getElementById('walletTechVersion');
    if(versionInfo) versionInfo.innerHTML = WS_VERSION;
    let tsVersionInfo = document.getElementById('bytechServiceVersion');
...
function initHandlers(){
    ...
    wsutil.liveEvent('a.external', 'click', (event) => {
        event.preventDefault();
        tech.openExternal(event.target.getAttribute('href'));
        return false;
    });
...
ipcRenderer.on('cleanup', () => {
    ...
    let htmlText = 'Terminating WalletTech...';
