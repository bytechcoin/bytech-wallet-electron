...
const DEFAULT_TITLE = 'WalletTech BytechCoin Wallet';
const SESSION_KEY = 'wltech';
...
var WalletTechSession = function(){
    if (!(this instanceof WalletTechSession)) return new WalletTechSession();
    ...
    this.sessDefault = {
        ...
        servicePath: settings.get('service_bin', 'bytech-service'),
        configUpdated: false,
...
WalletTechSession.prototype.get = function(key){
    key = key || false;
    if(!key){
...
WalletTechSession.prototype.getDefault = function(key){
    if(!key){
...
WalletTechSession.prototype.set = function(key, val){
    if(!this.sessDefault.hasOwnProperty(key)){
        throw new Error(`Invalid session key: ${key}`);
    }
...
WalletTechSession.prototype.reset = function(key){
    if(key){
        if(!this.sessDefault.hasOwnProperty(key)){
...
WalletTechSession.prototype.destroy = function(){
    return sessionStorage.removeItem(this.sessKey);
};

module.exports = WalletTechSession;
